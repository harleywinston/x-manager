...
func (db *UsersDB) GetUsersGroup(user models.Users) (models.Groups, error) {
	var err error
	user, err = db.GetUserFromDB(user)
	if err != nil {
		return models.Groups{}, err
	}
	var res models.Groups
	err = DB.Model(&models.Groups{}).Where("ID = ?", user.GroupID).Scan(&res).Error
	if err != nil {
...
func (db *UsersDB) GetUsersRecourse(user models.Users) (models.Resources, error) {
	group, err := db.GetUsersGroup(user)
	if err != nil {
